...
def list_tasks(
    ...
    if current_user.role.value == "level1":
        ...
        if assigned_to:
            try:
                query = query.filter(Task.assigned_to_id == uuid.UUID(assigned_to))
            except:
...
def create_task(
    ...
    if not payload.assigned_to_id:
        assigned_user = db.query(User).filter(User.role == "level1").first()

        if not assigned_user:
            raise HTTPException(status_code=400, detail="No Level1 user found")
...
def update_task(
    ...
    # ✅ FIX: Handle assignment update
    if "assigned_to_id" in data:
        try:
            user_uuid = uuid.UUID(str(data["assigned_to_id"]))
        except:
